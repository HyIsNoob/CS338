...
# Tìm checkpoint mới nhất hoặc tốt nhất trong thư mục
ckpt_dir = "updated_2_model_weights"
best_ckpt = os.path.join(ckpt_dir, "updated_2_model_weights(best).pth")
if not os.path.exists(best_ckpt):
    files = glob.glob(os.path.join(ckpt_dir, "*.pth"))
    if files:
        best_ckpt = max(files, key=os.path.getmtime)
    else:
        best_ckpt = "SpikeGPT-216M.pth"

print(f"Đang tải SpikeGPT từ: {best_ckpt}...")
